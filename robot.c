...
void robotAutoMotorMove(struct Robot * robot, int front_centre_sensor, int left_sensor, int right_sensor) {
    static int initial_turn_done = 0;

    if (initial_turn_done == 0) {
        robot->direction = LEFT;
        initial_turn_done = 1;
    }
	else if (front_centre_sensor == 0 && left_sensor == 0 && right_sensor == 0) {
    	// No walls detected in front, left, or right
    	if (robot->currentSpeed < 2)
            robot->direction = UP;
            initial_turn_done = 0;
	}
	else {
    	// There are walls detected
    	if (front_centre_sensor > 0) {
        	...
        	if (right_sensor == 0){
            	...
            	robot->direction = LEFT; // Turn left
        	}
        	else if (right_sensor == 0 && left_sensor >= 1){
            	robot->direction = RIGHT; // Turn right
            }
    	}
